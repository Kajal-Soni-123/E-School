...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>'.$row['email'].'</td>
      <td>
      <form action="" method="post" class="d-inline"><input type="hidden" value='.$student_id.' name="id"/>
   <button type="submit" class="btn btn-danger text-light" name="delete">delete</button></form>
<form action="edit_student.php" class="d-inline" method="post">
...
if(isset($_POST['id'])){
  $student_id=$_POST['id'];
  $delete_sql="DELETE FROM `register` WHERE user_id='$student_id'";
